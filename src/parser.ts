...
export class LogParser {
  ...
  private rawFile: string | null;

  constructor(opts: { silent?: boolean; rawFile?: string } = {}) {
    this.silent = opts.silent ?? false;
    this.rawFile = opts.rawFile ?? null;
    // Truncate the raw file so each run starts fresh
    if (this.rawFile) writeFileSync(this.rawFile, '', 'utf-8');
  }
  ...
  async parseFileStream(
    filePath: string,
    onProgress?: (sessions: Map<string, Session>) => void,
  ...
  watchFile(
    filePath: string,
    onUpdate: (sessions: Map<string, Session>) => void,
    intervalMs = 500,
  ): () => void {
    let lastSize = 0;
  ...
  private ingestLine(raw: string): boolean {
