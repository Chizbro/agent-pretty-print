...
export class MarkdownFormatter {
    ...
    fmtSession(s) {
        ...
        if (s.startTime)
            meta.push(`**Started:** ${new Date(s.startTime).toISOString()}`);
        if (s.endTime && s.startTime)
            meta.push(`**Duration:** ${fmtDur(s.endTime - s.startTime)}`);
        if (s.cwd)
            meta.push(`**CWD:** \`${s.cwd}\``);
        if (meta.length)
            out.push(meta.join('  \n'));
...
// ---- helpers ----
function groupToolCallsByModelCall(s) {
    const map = new Map();
...
function summarizeArg(tc, cwd) {
    ...
    if (cmd)
        return String(cmd).substring(0, 80);
